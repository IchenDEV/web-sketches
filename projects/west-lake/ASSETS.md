...
除三潭印月外，在线二十九景改用内置 ImageGen 生成的独立水墨画及分层素材。早期 GLB 和建模脚本保留作为历史素材，当前绘景不加载它们。

每景 `painted/<景点>/` 内包含：

- `source.webp`：该景原始水墨目标图，保留原画颜色与笔触。
- `background.webp`：移除最近的树木、建筑或岸石，并补全遮挡区域后的完整远景。
- `foreground-key.webp`：对应近景的色键轮廓；云栖竹径分为 `left-key.webp`、`right-key.webp` 两层。

图片使用无损 WebP 编码。色键图只决定透明覆盖范围，可见前景颜色从原画取样；色键的洋红色不进入画面。远景与前景铺在不同深度的曲面上，采用有限水平视差，不能绕到画中建筑背后。三潭印月继续使用原始模型、绘画贴图和实时反射水面。

分层生成使用每景原画作为输入，要求保持原始画幅、镜头、色调和地标位置。远景提示要求补全所有遮挡区域、保留已有远山和水面；近景提示要求保留指定主体的原始轮廓与位置，其他区域填充纯洋红色。原图保持不变。

## 总览与音乐

`previews/<景点>.webp` 为三十景实际渲染截图的600px缩略图；`previews/all-scenes.webp` 为完整总览长卷。仅打开景目后按需加载缩略图。

`audio/pei-lan.mp3`：古琴《佩兰》，Charlie Huang 演奏（丝弦，2006-05-28）。来源：[Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Pei_Lan.ogg)。选择其提供的 [CC BY 2.5](https://creativecommons.org/licenses/by/2.5/) 许可。仅从原始Ogg转码为96kbps MP3，未剪辑演奏；页面曲目信息中保留作者、来源、许可及转换说明。首次点击听琴才请求音频，切景不重启，音量单独保存。
