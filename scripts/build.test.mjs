...
test('collection links and previews resolve within a project Pages subpath', async () => {
  ...
  for (const link of links.filter(link => !/^(?:https?:|data:|#)/.test(link))) {
    assert.ok(!link.startsWith('/'), `Root-relative URL breaks project Pages: ${link}`);
...
test('built scene uses relative bundles and includes all runtime model and texture files', async () => {
  ...
  for (const [, link] of html.matchAll(/(?:src|href)="(\.\/assets\/[^\"]+)"/g)) {
    await stat(new URL(link, scene));
  }
  const source = await readFile(new URL('../projects/west-lake/main.js', site), 'utf8');
  const runtimeAssets = [...source.matchAll(/\$\{assetBase\}([^`]+)`/g)].map(match => match[1]);
